<?php
/**
 * Debug Import Final - ตรวจสอบปัญหาสุดท้ายในการ import
 * 
 * ใช้สำหรับ debug ปัญหาที่ทำให้ import หยุดที่ "เริ่มการ import ผ่าน Service..."
 */

echo "<h1>🔍 Debug Import Final - ตรวจสอบปัญหาสุดท้าย</h1>";
...
try {
    ...
    $testFile = $uploadDir . 'debug_import_final.csv';
    ...
    $csvContent .= "ทดสอบ,สุดท้าย,0812345678,test@example.com,123 ถ.ทดสอบ,เขตทดสอบ,จังหวัดทดสอบ,10000,สินค้าทดสอบ,1,1000,2024-01-15,ทดสอบสุดท้าย\n";
...
// 5. ทดสอบการ import ผ่าน Service แบบ step by step
echo "<h2>5. ทดสอบการ import ผ่าน Service แบบ step by step</h2>";

try {
    echo "🔍 เริ่มการ debug importSalesFromCSV...<br>";
    
    // เรียกใช้ method โดยตรงและดู error ที่เกิดขึ้น
    echo "🔍 เรียกใช้ importSalesFromCSV...<br>";
...
// 10. ทดสอบการ import ผ่าน Controller
echo "<h2>10. ทดสอบการ import ผ่าน Controller</h2>";

try {
    echo "🔍 เริ่มการ debug importSales method...<br>";
    
    // สร้างไฟล์ upload จำลอง
    $uploadFile = $uploadDir . 'debug_controller_final.csv';
    copy($testFile, $uploadFile);
    
    // จำลอง $_FILES
    $_FILES['csv_file'] = [
        'name' => 'debug_controller_final.csv',
        'type' => 'text/csv',
        'tmp_name' => $uploadFile,
        'error' => UPLOAD_ERR_OK,
    ...
    echo "🔍 เรียกใช้ importSales method...<br>";
...
try {
    $filesToDelete = [$testFile, $uploadDir . 'debug_controller_final.csv'];
