...
<body>
								...
								<div class="row">
									<div class="col-md-6">
										...
										<p><strong>สถานะการชำระเงิน:</strong>
											<select id="paymentStatusSelect" class="form-select form-select-sm d-inline-block" style="max-width: 180px;" onchange="updatePaymentStatusInline(<?php echo (int)$order['order_id']; ?>, this.value)">
												<?php
													$payStatuses = ['pending'=>'รอชำระ','paid'=>'ชำระแล้ว','partial'=>'ชำระบางส่วน','cancelled'=>'ยกเลิก','returned'=>'ตีกลับ'];
													$curr = $order['payment_status'] ?? 'pending';
													foreach ($payStatuses as $val=>$label) {
														$sel = ($curr === $val) ? 'selected' : '';
											...
											?>
											<span id="paymentStatusBadge" class="badge bg-<?php echo $payColor; ?> text-dark ms-2"><?php echo $payMap[$order['payment_status'] ?? 'pending']; ?></span>
										</p>
									</div>
									<div class="col-md-6">
	...
	<script>
		function reflectPaymentBadge(status) {
			...
			badge.className = 'badge bg-' + color + ' text-dark ms-2';
